...
const ParagonList: React.FunctionComponent<IParagonListProps> = (props) => {
  ...
  useEffect(() => {
    if (!isLoaded) {
      axios
        .get(`http://sodfestival.online/api/data1?_sort=id&_order=desc&_limit=30`)
        .then((res) => {
          setData1(res.data);
        })
        .catch((err) => console.log(err));
      axios
        .get(`http://sodfestival.online/api/data2?_sort=id&_order=desc&_limit=30`)
        .then((res) => {
          setData2(res.data);
        })
        .catch((err) => console.log(err));
      axios
        .get(`http://sodfestival.online/api/data3?_sort=id&_order=desc&_limit=30`)
        .then((res) => {
          setData3(res.data);
        })
        .catch((err) => console.log(err));
